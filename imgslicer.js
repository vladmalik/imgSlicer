(function($) {
	...
	$.fn.imgslicer = function(customOptions) {
		...
		if($.data(document.body, "imgslicerIndex")) {
			savedIndex = $.data(document.body, "imgslicerIndex");
			$.data(document.body, "imgslicerIndex", thisCount + savedIndex);
		}
		else {
			$.data(document.body, "imgslicerIndex", thisCount);
		}
		...
		return this.each(function(index, value) { //imgIndex counts each instance of an image on the page
			...
			img.wrap("<div style='position:relative' class ='imgWrapper' id='imgWrapper-" + (savedIndex + index) + "'/>");
			...
			for(r=0;r<options["rows"];r++) {
				for(c=0;c<options["cols"];c++) {
					imgSlices +="<div class='imgSlice' id='imgSlice-" + (c+1) + "-" + (r+1) + "' style='width:" + colWidth + "px; height:" + rowHeight + "px; background: url(\"" + imgSrc + "\") -" + c*colWidth + "px -" + r*rowHeight +"px; float: left; position: relative;'></div>";
				}
			}
